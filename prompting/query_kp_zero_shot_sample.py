...
def main():
    ...
    for entry in tqdm(data, desc="Querying"):
        ...
        if backend == 'openai':
            ...
            print(entry['goal'] + ': ' + ' ; '.join(entry['keyphrases']))
            print(json.dumps([x['message']['content'] for x in response['choices']], indent=4))
            
            sample_result = []
            for x in response['choices']:
                if x['finish_reason'] == "length":
                    sample_result.append(','.join(x['message']['content'].strip().split(',')[:-1]))
                else:
                    sample_result.append(x['message']['content'].strip())
            all_outs.append(sample_result)

        print(prompt)

    print('total token used:', token_tot)
